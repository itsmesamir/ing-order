    ...
    <div className="p-4">
      <div className="flex gap-x-4 items-center justify-between">
        <div className="flex gap-x-4">
          <button
            type="button"
            className={classNames('bg-primary-600 h-9 rounded w-14 text-white', {
              '': isFirstButtonDisabled,
            })}
            onClick={() => onButtonClick(isFirstButtonDisabled, MINIMUM_PAGE_COUNT)}
          ...
          <button
            type="button"
            className={classNames('bg-primary-600 h-9 rounded w-14 text-white', {
              'text-grey-900 bg-grey-100': isPrevButtonDisabled,
            })}
          ...
          <button
            type="button"
            className={classNames('bg-primary-600 h-9 rounded w-14 text-white', {
              'text-grey-900 bg-grey-100': isNextButtonDisabled,
            })}
          ...
          <button
            type="button"
            className={classNames('bg-primary-600 h-9 rounded w-14 text-white', {
              'bg-primary-600 h-9 rounded w-14 text-white--active': isLastButtonDisabled,
              'text-grey-900 bg-grey-100': pageCount <= DEFAULT_PAGE,
            })}
        ...
        <div className="flex gap-x-4">
          <select
            ...
            className="outline-none border-grey-300 rounded"
          >
            {PAGE_SIZE_OPTIONS.map(pageSizeOption => (
              <option className="text-grey-900 " key={pageSizeOption} value={pageSizeOption}>
                Show {pageSizeOption}
              </option>
            ))}
          </select>
