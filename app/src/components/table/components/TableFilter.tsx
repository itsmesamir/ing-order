...
export interface BaseTableFilterProps<T> {
  ...
  styles?: {
    dropdownCustomStyles: StylesConfig<FilterData, boolean, GroupBase<FilterData>>;
  };
  singleFilter?: {
    dropdown?: boolean;
...
function ActionContent(props: ActionContentProps) {
  ...
  return (
    <>
      <div className={classNames('flex gap-x-4', classes?.container)}>
        <button
          ...
          className={classNames(
            'btn-primary',
            {
              'border-grey-300': !isFilterChanged,
              '': isFilterChanged,
            },
            classes?.primary
          )}
      ...
      {trailing && <div className="d-flex items-center ml-auto">{trailing}</div>}
    </>
  );
}

function CustomMenulist(props: MenuListProps, additionalProp: ActionContentProps) {
  const {
    isFilterChanged,
    onFilterApply,
    selectedFilters,
...
export default function TableFilters<T>(props: TableFilterProps<T>) {
  ...
  const handleToggleSwitchFilterChange = (e: React.ChangeEvent<HTMLInputElement>, key: string) => {
    if (!e || !e.target) {
      return;
    }
  ...
  const FilterElement = useCallback(
    ({ filter, activeFilters, isAdditionalFilter = false, isFiltersLoading = false }: Any) => {
      if (filter.isHidden) {
        return null;
      }
      ...
      return (
        ...
        >
          <Dropdown
            ...
            value={
              activeFilters &&
              findDefaultFilters(filter.options as FilterData[], activeFilters[filter.key])
            }
            ...
            customStyles={{
              ...
              control: (base: CSSObjectWithLabel, { isFocused }: { isFocused: boolean }) => ({
                ...base,
                width: '180px',
                flex: 1,
                height: '40px',
              ...
              valueContainer: (base: CSSObjectWithLabel) => ({
                ...
                height: 'auto', // Adjust value container height
              ...
              multiValueRemove(
                ...
                state: ClearIndicatorProps<Any, boolean, GroupBase<Any>>
              ) {
                return {
                  ...base,
                  color: neutrals900,
  ...
  return (
    <>
      ...
      <div className={classNames('table-filters', className)}>
        ...
        <div className={classNames('max-lg:hidden table__header d-flex', classes?.header)}>
          <div className={classNames('flex flex-wrap gap-4', classes?.content)}>
            <div className={classNames('flex items-center flex-wrap gap-4', classes?.filterList)}>
              {fixedFilters.map((filter, idx) => (
